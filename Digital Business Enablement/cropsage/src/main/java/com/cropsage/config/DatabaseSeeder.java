...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Usuario u1 = Usuario.builder().nome("nome1").email("email1").senha(encoder.encode("senha1")).build();
        Usuario u2 = Usuario.builder().nome("nome2").email("email2").senha(encoder.encode("senha2")).build();
        Usuario u3 = Usuario.builder().nome("nome3").email("email3").senha(encoder.encode("senha3")).build();
